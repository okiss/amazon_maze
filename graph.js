...
function* findPaths(adjList, partialPath, endVertices) {
  ...
  for (let i = 0; i < neighbors.length; i++) {
    ...
    const shouldExplore = nextVertex !== currentVertex
...
function* findAllOpenPaths(adjList, vertexList) {
  const startVertices = Object.keys(vertexList).filter((vertex) => vertexList[vertex].start);
  const endVertices = Object.keys(vertexList).filter((vertex) => vertexList[vertex].end);
